...
def run_producer():
    p = Producer({'bootstrap.servers':'pkc-4r297.europe-west1.gcp.confluent.cloud:9092',
                  ...
                  'sasl.username':'XB5DPS6HMXDAVUTB',
                  'sasl.password':'GoMrfSd1x9UZWQ/MhBzIg1qLMlXEQW0Xwj28UhWdGcqjW9wTSMDGn5XCak+Az9fF',
                  'acks':'-1',
...
def write_to_queue(p,lat, lon):
    while True:
            try:
                response = requests.get("http://api.openweathermap.org/data/2.5/air_pollution?lat="+str(lat)+"&lon="+str(lon)+"&appid=0170303693f51ac954ab9e1f360d2dc9")
                print(response.json())
                if (response.json!=401) & (response!=400):
                    msg_value=response.json()
                    msg_header = {"source" : b'DEM'}
                    p.poll(timeout=0)
                    p.produce(topic='AirQuality', value=str(msg_value), headers=msg_header, on_delivery=delivery_report)
                break
